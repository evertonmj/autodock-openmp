...
void Genetic_Algorithm::crossover(Population &original_population, int outlev, FILE *logFile)
{
   ...
   for (i=0; i<original_population.num_individuals(); i++) {
      ...
      // ignlgi_t is GeNerate LarGe Integer and is in com.cc
      #ifdef _OPENMP
      ...
      if (ranf() < c_rate) {
         ...
         switch(c_mode) {
            case OnePt:
                ...
                do {
                    // first_point = ignlgi_t()%original_population[i].genotyp.num_genes();
...
//#define DEBUG
   ...
   while ( start_index < num_indiv && --emergencydoor>0 ) {
        #ifdef _OPENMP
	i = ignlgi_t(omp_get_thread_num())%num_indiv;
  	#else
	i = ignlgi()%num_indiv;
	#endif
...
int Genetic_Algorithm::search(Population &solutions, Eval *evaluate, int outlev, FILE *logFile)
{
   ...
   if (elitism > 0) {
	  ...
	  int index_individuals = 0;
	  index_individuals = solutions.num_individuals();
      for (i=0; i<elitism; i++) {
         newPop[index_individuals-1-i] = solutions[i];
      }
   }
