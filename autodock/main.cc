...
//POINT WHERE CODE STUCK
	    ...
	    // #pragma omp parallel for private(j) shared(crdpdb, nconf) schedule(static)
        #pragma omp parallel for private(j) schedule(static)
	    // #pragma omp parallel for private(j) schedule(static) num_threads(24)
...
/* MPique TODO*/
		 ...
		 *   M Pique Oct 2013
		 */
		if(j==0&&nconf==0) getsd_t(&runseed[0][0], &runseed[0][1], thread_num);
		setsd_t(runseed[nconf+j][0], runseed[nconf+j][1], thread_num); 
		...
		if(outlev>=LOGRUNV)
                ...
                sHist[nconf+j] = call_glss( GlobalSearchMethod, LocalSearchMethod,
                                          ...
                                          B_RandomTran0, B_RandomQuat0, B_RandomDihe0,
                                          info, FN_pop_file, end_of_branch, thread_num);
	        ...
	        printdate(logFile, 2 );
	 		...
	 		//Start Particle Swarm Optimization Run	               	 		
                sHist[nconf+j] = call_glss( GlobalSearchMethod, LocalSearchMethod,
                                          ...
                                          output_pop_stats, &ligand, &evaluate,
                                          B_RandomTran0, B_RandomQuat0, B_RandomDihe0,
                                          info, FN_pop_file, end_of_branch, -1 /*thread_num used on openmp*/);
        ...
        if (ntor > 0) {
           (void) sprintf( message, "%s: WARNING: Using autodock4.0 unbound extended model in autodock4.2!\n", programname );
            ...
            sUnbound_ext = call_glss( GlobalSearchMethod, LocalSearchMethod,
                                      ...
                                      B_RandomTran0, B_RandomQuat0, B_RandomDihe0,
                                      info, FN_pop_file, end_of_branch , -1 /*thread_num for openmp*/);
...
static void set_seeds( FourByteLong seed[2], char seedIsSet[2], FourByteLong runseed[MAX_RUNS][2], const int outlev, FILE *logFile ) {
	 ...
	 #ifdef _OPENMP  
		runseed[j][0] = ignlgi_t(omp_get_thread_num());
		runseed[j][1] = ignlgi_t(omp_get_thread_num());
         #else
		runseed[j][0] = ignlgi();
		runseed[j][1] = ignlgi();
         #endif
